...
- `EC2_APP_DIR`: defaults to `/opt/trading-bot/app`
...
```bash
sudo mkdir -p /opt/trading-bot/app
sudo chown -R "$USER":"$USER" /opt/trading-bot
mkdir -p /opt/trading-bot/app
```
...
```bash
ls -la /opt/trading-bot/app/logs
tail -n 50 /opt/trading-bot/app/logs/live_cron.log
tail -n 50 /opt/trading-bot/app/logs/paper_cron.log
```
